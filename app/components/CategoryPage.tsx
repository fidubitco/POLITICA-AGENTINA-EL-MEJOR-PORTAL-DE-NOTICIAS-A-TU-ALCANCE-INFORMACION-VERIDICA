...
import { Noticia } from '../data/noticias-completas';
...
interface CategoryPageProps {
  ...
  description: string;
  noticias: Noticia[];
  trendingTopics: Array<{ name: string; count: string }>;
}
...
export default function CategoryPage({
  ...
  const topNews = noticias.slice(1, 7);
  const moreNews = noticias.slice(7);
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <nav className="border-t border-gray-200 no-print">
          <div className="container mx-auto px-4">
            <ul className="flex items-center gap-6 py-3 overflow-x-auto">
              ...
              <li><Link href="/politica" className={`nav-link ${categorySlug === 'politica' ? 'nav-link-active' : ''}`}>Política</Link></li>
              <li><Link href="/economia" className={`nav-link ${categorySlug === 'economia' ? 'nav-link-active' : ''}`}>Economía</Link></li>
              <li><Link href="/judicial" className={`nav-link ${categorySlug === 'judicial' ? 'nav-link-active' : ''}`}>Judicial</Link></li>
              <li><Link href="/internacional" className={`nav-link ${categorySlug === 'internacional' ? 'nav-link-active' : ''}`}>Internacional</Link></li>
              <li><Link href="/sociedad" className={`nav-link ${categorySlug === 'sociedad' ? 'nav-link-active' : ''}`}>Sociedad</Link></li>
            </ul>
          </div>
        </nav>
      </header>
      ...
      <main className="container mx-auto px-4 py-8">
        {/* Category Header */}
        <div className={`mb-8 pb-6 border-b-4 border-${categoryColor}-600`}>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section>
              <h3 className="text-2xl font-serif font-bold mb-6 pb-3 border-b-2 border-gray-900">
                Noticias Destacadas de {categoryName}
              </h3>
                ...
                {topNews.map((article) => (
                  <article key={article.id} className="article-card group cursor-pointer">
                    ...
                    <div className="p-4">
                      ...
                      <h4 className={`article-title mt-3 group-hover:text-${categoryColor}-700 transition-colors line-clamp-2`}>
                        {article.title}
                      </h4>
                ...
                {moreNews.map((article) => (
                  <article key={article.id} className="flex flex-col md:flex-row gap-4 pb-4 mb-4 border-b border-gray-200 last:border-0">
                    ...
                    <div className="flex-1">
                      ...
                      <h4 className={`article-title mt-2 hover:text-${categoryColor}-700 transition-colors line-clamp-2`}>
                        {article.title}
                      </h4>
          ...
          <aside className="space-y-6">
            ...
            <div className="sidebar-widget bg-gray-50">
              <h3 className="sidebar-title">
                Más Leídas en {categoryName}
              </h3>
              <ol className="space-y-4">
                {noticias.slice(0, 5).map((article, index) => (
                  <li key={article.id} className="flex gap-3">
                    <span className={`text-3xl font-bold text-${categoryColor}-600`}>{index + 1}</span>
                    <div className="flex-1">
                      <h4 className={`text-sm font-semibold text-gray-900 hover:text-${categoryColor}-700 transition-colors line-clamp-2 cursor-pointer`}>
                        {article.title}
                      </h4>
            ...
            {/* Newsletter */}
            <div className={`sidebar-widget bg-${categoryColor}-50 border-${categoryColor}-200`}>
              <h3 className={`sidebar-title text-${categoryColor}-900 border-${categoryColor}-900`}>
                Newsletter {categoryName}
              </h3>
              <p className="text-sm text-gray-700 mb-4">
                Recibe las noticias de {categoryName.toLowerCase()} más importantes en tu email
              </p>
              <form className="space-y-3">
                ...
                <button
                  type="submit"
                  className={`w-full bg-${categoryColor}-600 text-white py-2 font-semibold hover:bg-${categoryColor}-700 transition-colors`}
                >
                  Suscribirse
                </button>
              </form>
